...
| `2b_fixed_reactor_theta13.ipynb` | Can a *fixed* near reactor replace the movable one? Vertex Monte Carlo of the $L/E$ structure inside the JUNO volume, four standoffs 50 m–1.5 km with the far-reactor $\Delta m^2_{ee}$ prior — verdict: the 33 m volume gives only 0.026 rad of phase, a single fixed unit is flux-shape-limited at $\sim2°$; a second fixed unit as flux monitor recovers $0.3$–$0.5°$. |
...
| `8_neutron_shielding.ipynb` | Feasibility: the Watt-spectrum neutron and core-gamma leakage of the parked reactor through its package and JUNO's 4 m pool water (removal-cross-section estimate) — a research-reactor-class shield (~6 m water-eq / ~2 m steel + sub-metre high-Z) renders both negligible for single-hit physics. |
| `7_alps.ipynb` | Axion-like particles from the parked reactor: Primakoff/Compton production in the core (exact cross sections, validated against independent implementations), detection in JUNO via inverse processes and in-flight decays through the 16 kt volume — the **cosmological triangle** covered in full at 90% CL, background included. |
| `6_sterile_wiggles.ipynb` | Sterile neutrinos with the parked reactor: the finite detector size turns JUNO into a many-baseline experiment — vertex-binned wiggles reach $\sin^22\theta_{14}\sim10^{-3}$ at the eV$^2$ scale, $\sim20\times$ beyond the integrated analysis, immune to flux systematics by construction. |
| `5_near_sm_precision.ipynb` | A fixed 10 MW HALEU reactor at 50 m: the IBD spectrum, the E$\nu$ES spectrum in $E_\nu$ and $T_{\rm recoil}$ (NEPTUNE cross sections, exact flavor decomposition), the IBD single-hit background, and the reach on $\sin^2\theta_W$ and $(g_V, g_A)$ vs MW$\cdot$yr — E$\nu$ES alone is flux-shape-limited at $\sigma\approx0.005$; the joint IBD+E$\nu$ES fit anchors the flux in situ against the $0.2\%$ IBD cross section and reaches $\sigma(\sin^2\theta_W)\approx0.001$. |
...
| `reactor.shielding` | Watt spectrum, energy-dependent removal cross sections and NIST gamma attenuation for layered shields; rate-into-LS and required-thickness solvers for the parked reactor. |
| `reactor.alps` | ALP production at the reactor (FRJ-1 photon flux, exact Primakoff + Compton-like conversion, XCOM-anchored SM normalisation) and JUNO detection (inverse processes, decays, survival/decay probabilities, counting $\chi^2$), following arXiv:2511.01812. |
| `reactor.solar` | Solar fluxes (B8/hep/CNO continua + pep line) and the adiabatic LMA survival probability, shared by the near-reactor studies. |
| `reactor.sterile` | `SterileNearReactor`: 3+1 disappearance across the fiducial volume — analytic baseline distribution, joint IBD+E$\nu$ES $(L, E)$ binning, per-$L$-bin uniformity systematics, and analytic exclusion curves via $\Delta\chi^2 = \sin^42\theta_{14}\,Q(\Delta m^2)$. |
| `reactor.near_sm` | `FixedNearReactor`: a parked HALEU source — IBD and E$\nu$ES spectra (measured U235 flux, NEPTUNE cross sections with free $(g_V,g_A)$, atomic stepping correction), IBD-singles + solar + standard-JUNO backgrounds, energy-scale/evolution systematics, and Asimov sensitivities for $\sin^2\theta_W$ and the coupling plane. |
| `reactor.fixed_theta13` | `VertexMonteCarlo` (vertices, $1/L^2$ weights, core, resolution), `FixedReactorTheta13` (binned $(L,E)$ Fisher analysis with the far-reactor $\Delta m^2_{ee}$ prior), `TwoFixedReactorsTheta13` (near flux monitor + far unit). |
